...
CASE_SinDecoder::CASE_SinDecoder(bool *isRUNNING_DECODER, QObject *parent) :
...
//从界面的参数配置框中读取参数
void CASE_SinDecoder::OnRunPara(QList<QString> slist)
{
    mchsin = slist.at(0);
    savepath = slist.at(1);
}

void CASE_SinDecoder::cyclestart()
{
    contFlag++;
    qDebug()<<"countflag:"<<contFlag;
    sended=1;
}
...
extern char outputpathAOS[fileNameLen]; //输出AOS帧数传包数据路径
extern char outputpathSourcepackage[fileNameLen]; //输出源包数据路径
extern char outputpath2711package[fileNameLen]; //输出2711源包数据路径
extern char outputpath2711sub[fileNameLen]; //输出2711子包数据路径
extern char outputpath2711subsub[fileNameLen]; //输出2711子子包路径


void CASE_SinDecoder::run()
{
...
//    FILE			*fpIn1, *fpIn2, *fptest, *fpOut;
      FILE			*fpIn1, *fptest, *fpOut;
//    __int64			filelength1 = -1, filelength2 = -1, BUFFSIZE, PROCESSSIZE;
    __int64			filelength1 = -1, BUFFSIZE, PROCESSSIZE;
...
//    unsigned int JGid = 0;
//    /*JH100数据*/
//    unsigned int JG100id = 0;
//    /*XSCL数据*/
//    unsigned int GYIPDid = 0 , JGIPDid = 0, GYTWDWXXid = 0 , JGTWDWid = 0;
//    unsigned int GYGTWid = 0 , GYYDXXid = 0, GYTXCLRZid =0, TWDWRZid = 0;
//    unsigned int YDGZRZid = 0, JGTXCLRZid = 0;
...
//    PROCESSSIZE=(128*1024*frame_len);
    ...
    PROCESSSIZE=(8 *1024*frame_len);//根据电脑可分配内存适当更改
    ...
    //GY
    unsigned char *SSXWGCout= NULL, *XWGCout= NULL, *_GYAout= NULL,*_GYBout= NULL, *_GYCout= NULL, *_JGout= NULL, *JG100out= NULL, *XSCL01out= NULL, *XSCL02out= NULL;
    //受保护的GY相机图像
...
//    strSSZXGC = (unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));//
//    strSSRWSJ = (unsigned char*)malloc( 2*PROCESSSIZE * sizeof(unsigned char));//
//    strSSDKCB = (unsigned char*)malloc( 2*BUFFSIZE * sizeof(unsigned char));//
//    strSSDKZCB = (unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));//
//    strSSDDDPD = (unsigned char*)malloc(2*BUFFSIZE * sizeof(unsigned char));//
//    strSSDDDB = (unsigned char*)malloc( 2*PROCESSSIZE * sizeof(unsigned char));//
//    strSSXJCS = (unsigned char*)malloc( 2*PROCESSSIZE * sizeof(unsigned char));//
//    strSSJGYY = (unsigned char*)malloc(2* PROCESSSIZE * sizeof(unsigned char));//

//    ZXGCout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    RWSJout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    DKCBout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    DKZCBout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    DDDPDout=(unsigned char*)malloc(2*BUFFSIZE * sizeof(unsigned char));
//    DDDBout=(unsigned char*)malloc(2*BUFFSIZE* sizeof(unsigned char));
//    XJCSout=(unsigned char*)malloc(2*PROCESSSIZE* sizeof(unsigned char));
//    JGCSout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    JGYYout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));

//    SSZXGCout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    SSRWSJout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    SSDKCBout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    SSDKZCBout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    SSDDDPDout=(unsigned char*)malloc(2*BUFFSIZE * sizeof(unsigned char));
//    SSDDDBout=(unsigned char*)malloc(2*BUFFSIZE* sizeof(unsigned char));
//    SSXJCSout=(unsigned char*)malloc(2*PROCESSSIZE* sizeof(unsigned char));
//    //JGCSout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    SSJGYYout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));

//    DDDPDKC2711out=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    DDDB2711out=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    DKCB2711out=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    DKZCB2711out=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
    ...
    _JGout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
    ...
    JGout=(unsigned char*)malloc(2*PROCESSSIZE * sizeof(unsigned char));
//    std::string     inputfile1_stdstr, inputfile2_stdstr;

    std::string     inputfile1_stdstr;
    ...
    inputfile1_stdstr = textCodec->fromUnicode(mchsin).data();
...
//    filelength2 = filesize64(fpIn2);
    ...
    MainWindow::fakeUi->setLinetext(QString("文件1码流长度为 %1 ").arg(filelength1));
    emit stateSig("success", QString("文件打开成功! 文件1码流长度为 %1").arg(filelength1));
    ...
    *isRUNNING_DECODER = true;
    ...
    if (NULL == strIn1 || NULL == strremain1)
    {
        MainWindow::fakeUi->setLinetext(QString("check: failed to apply for AOS input space!!\n"));
        return;
    }
    //20210330 kj
    if (NULL == SSdata || NULL == HFdata || NULL == TCZdata  || NULL == remaincode1 )
    {
...
//                iSumLen2 += skip_len;
            ...
            else
            {
                ...
                //y
//                if (iShiftLen1 == BUFFSIZE )
//                    contFlag++;
////                else if (iShiftLen1 == 0 )
////                    contFlag++;
//                else
//                    contFlag = 0;
...
//                        remaincode1, remaincode2, &remainlen1, &remainlen2, 0, record_len, frame_len, outputpath,vcNum,vcstate,&synvc,&synflag);
                ...
                newcombine_one(strIn1, iProcLen1,
                             SSdata,HFdata,TCZdata,
                             &SSlen, &HFlen,&TCZlen,
                             remaincode1,&remainlen1,
                             1024, 1024);
...
//                iSumLen2 += (iProcLen2 - iShiftLen2);
                MainWindow::fakeUi->setLinetext(QString("该次执行buff中文件1剩余 %1").arg(iShiftLen1));
                MainWindow::fakeUi->setLinetext(QString("该次执行后文件1总执行量为 %1").arg(iSumLen1));
                MainWindow::fakeUi->setLinetext(QString("该次执行后文件1总执行量剩余 %1").arg(filelength1 - iSumLen1));
...
//                    CASE_SID::fakeUi->setLinetext_1(QString("%1").arg(outwords));
                    ...
                    {
                        //XWGC
                        //dealSourcePackage->dealZXGCPackage(strZXGC, ZXGClen,&comZXGC, &obtainedSourceStream,ZXGCout,&rmHFXWGClen,0);
