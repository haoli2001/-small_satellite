...
RealtimeDecoder::RealtimeDecoder(CCycleBuffer *buffer1, CCycleBuffer *buffer2, QReadWriteLock *readlock1, QReadWriteLock *readlock2, QObject *parent) : QThread(parent)
  , buffer1(buffer1), buffer2(buffer2), readlock1(readlock1), readlock2(readlock2)
{
...
void RealtimeDecoder::OnRunPara(QString path)
{
    this->savepath = path;
}

void RealtimeDecoder::Onend1Slot()
{
    Recved1ended = true;
};
void RealtimeDecoder::Onend2Slot()
{
    Recved2ended = true;
}

void RealtimeDecoder::cyclestart()
{
    sended=1;
};
...
void RealtimeDecoder::run()
{
    ...
    __int64			filelength1 = 0, filelength2 = 0, BUFFSIZE, PROCESSSIZE;//l 0627 -1修改为0
...
//    PROCESSSIZE=(128*1024*frame_len);
    ...
    {
        savepath = savepath + "realtime-" + data_time;
    }
    else
    {
        savepath = savepath + "\\" + "realtime-" + data_time;
    ...
    emit InitReadySig();
    int test = 0;
    __int64 deallen1 = 0, deallen2 = 0;
    __int64 readlen1 = 0, readlen2 = 0;
    while(1)
    {
        if(send)
        {
            ...
            //        buffer1->empty();
            //        buffer2->empty();
            if(Recved1ended && Recved2ended && buffer1->isEmpty() && buffer2->isEmpty())
                break;
...
//            }
            ...
            if(Recved2ended && buffer2->isEmpty())
            {
                synflag = 3;
            }
            if(Recved1ended && Recved2ended)
            {
                if(buffer1->isEmpty()||buffer2->isEmpty())
                {
                    int test = 0;
            ...
            if(buffer1->getLength() < BUFFSIZE || buffer2->getLength() < BUFFSIZE)
            {
                if(!Recved1ended && !Recved2ended)
                    continue;
            }
...
//                while(readlen2 < skip_len)
//                {
//                    readlock2->lockForRead();
//                    readlen2 += buffer2->read(strSkip2, skip_len - readlen2);
//                    readlock2->unlock();
//                    if(buffer2->isEmpty() && Recved2ended)
//                        break;
//                }
//                firstflag = 0;
//                iSumLen1 += skip_len;
//                iSumLen2 += skip_len;
            ...
            else
            {
                ...
                while(readlen2 < deallen2)
                {
                    readlock2->lockForRead();
                    readlen2 += buffer2->read(strIn2 + iShiftLen2 + readlen2, deallen2 - readlen2);
                    readlock2->unlock();
                    if(buffer2->isEmpty() && Recved2ended)
                        break;
                }
