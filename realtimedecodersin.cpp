...
RealtimeDecodersin::RealtimeDecodersin(CCycleBuffer *buffer1, QReadWriteLock *readlock1, QObject *parent) : QThread(parent)
  , buffer1(buffer1), readlock1(readlock1)
{
...
void RealtimeDecodersin::OnRunPara(QString path)
{
    this->savepath = path;

}

void RealtimeDecodersin::Onend1Slot()
{
    Recved1ended = true; //初始值为FALSE
}

void RealtimeDecodersin::cyclestart()
{
    sended=1;
}
...
//    __int64			filelength1 = -1, filelength2 = -1, BUFFSIZE, PROCESSSIZE;
    __int64			filelength1 = 0, BUFFSIZE, PROCESSSIZE;
...
//    std::string     inputfile1_stdstr, inputfile2_stdstr;
    ...
    {
        savepath = savepath + "realtimesin-" + data_time;
    }
    else
    {
        savepath = savepath + "\\" + "realtimesin-" + data_time;
    }
    //emit dialogTitleSig(savepath);
    ...
    std::string     savepath_stdstr;

    savepath_stdstr = textCodec->fromUnicode(savepath).data();

    strcpy(outputpath, savepath_stdstr.c_str());
    mkdir(outputpath);
    ...
    sprintf(outputpath2711package, "%s/imagepackage", outputpath);
    ...
    while(1)
    {
        if(sended)
        {
            ...
            //if(Recved1ended && Recved2ended && buffer1->isEmpty() )
            //buffer1->empty();
            if(Recved1ended  && buffer1->isEmpty() )
                break;

            if(buffer1->getLength() < BUFFSIZE )
            {
                if(!Recved1ended )
                    continue;
            }
            ...
            {
                readlen1 =  0;
//                while(readlen1 < skip_len)
//                {
//                    readlock1->lockForRead();
//                    readlen1 += buffer1->read(strSkip1, skip_len - readlen1);
//                    readlock1->unlock();
//                    if(buffer1->isEmpty() && Recved1ended)
//                        break;
//                }
...
//                iSumLen1 += skip_len;
            ...
            else
            {
                ...
                readlen1 = 0;
                deallen1 = BUFFSIZE - iShiftLen1;

                while(readlen1 < deallen1)
                {
                    readlock1->lockForRead();
                    readlen1 += buffer1->read(strIn1 + iShiftLen1 + readlen1, deallen1 - readlen1);
                    readlock1->unlock();
                    if(buffer1->isEmpty() && Recved1ended)
                        break;
                }
                ...
                //处理buff
                iProcLen1 = readlen1 + iShiftLen1;

                filelength1 += iProcLen1;
